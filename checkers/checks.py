...
def check_model_has_description(model: Model, params: Dict):
    ...
    assert len(model.description) >= params['minimum_description_length'], "Model description not long enough"
    assert len(model.description.split()) >= params['minimum_description_words'], f"Model description is too few words"
